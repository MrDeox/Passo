...
def test_rh_autocreates_executor_for_task(reset_state): # Renamed for clarity
    ...
    ed.criar_local("Sala Comum", "Local para agentes", [])
    # ed.adicionar_tarefa now creates Task objects.
    # The original test for adicionar_tarefa implies it adds to ed.tarefas_pendentes
    task = ed.adicionar_tarefa("Resolver problema X")
    assert task in ed.tarefas_pendentes
    assert task.status == "todo"

    ed.saldo = 50 # Suficiente para contratar
    # rh.saldo = ed.saldo # rh.py directly uses ed.saldo now

    rh.modulo_rh.verificar()

    assert len(ed.agentes) > 0, "Nenhum agente foi criado pelo RH."

    created_executor = next((ag for ag in ed.agentes.values() if ag.funcao == "Executor"), None)
    ...
    # The original check `assert ed.tarefas_pendentes == []` might be too strict if other tasks could exist.
    ...
    assert task not in ed.tarefas_pendentes, "Tarefa processada não foi removida da lista de pendências do RH."


class TestExecutorIdleLogic:

    def test_executor_cycles_idle_increment(self, reset_state):
        local = ed.criar_local("Escritorio", "Local de trabalho", [])
        executor = ed.criar_agente("ExecutorOcioso", "Executor", "gpt-4", local.nome, objetivo_atual="Aguardando novas atribuições.")
    ...
    def test_executor_dismissal_after_5_idle_cycles(self, mock_random, reset_state):
        ...
        local = ed.criar_local("SalaDispensa", "Local de teste", [])
        executor_name = "ExecutorParaDispensar"
        executor = ed.criar_agente(executor_name, "Executor", "gpt-4", local.nome, objetivo_atual="Aguardando novas atribuições.")
        executor.cycles_idle = 4 # Prepara para atingir 5 no próximo ciclo

        rh.modulo_rh.verificar() # Ciclo 5 -> dispensa

        assert executor_name not in ed.agentes, f"Executor {executor_name} não foi dispensado."
    ...
    def test_executor_role_change_after_5_idle_cycles(self, mock_random, reset_state):
        ...
        local = ed.criar_local("SalaRedefinicao", "Local de teste", [])
        ...
        assert executor_name in ed.agentes, "Executor foi removido em vez de redefinido."
        redefined_agent = ed.agentes[executor_name]
    ...
    def test_non_executor_not_affected_by_idle_logic(self, reset_state):
        local = ed.criar_local("SalaNormal", "Local de trabalho", [])
        ceo_agent = ed.criar_agente("Chefe", "CEO", "gpt-4", local.nome, objetivo_atual="Aguardando novas atribuições.")
        ceo_agent.cycles_idle = 10 # Simula ociosidade alta

        rh.modulo_rh.verificar()

        assert "Chefe" in ed.agentes # Não deve ser demitido
    ...
    def test_executor_with_valid_objective_not_laid_off(self, reset_state):
        local = ed.criar_local("SalaTrabalho", "Local de trabalho", [])
        task_id = "task_abc"
        executor = ed.criar_agente("ExecutorOcupado", "Executor", "gpt-4", local.nome, objetivo_atual=f"task_id:{task_id}")
        ...
        assert "ExecutorOcupado" in ed.agentes
