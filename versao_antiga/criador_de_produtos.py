...
# registrar_evento will be imported from state below, or use state.registrar_evento
...
# PRODUTOS_GERADOS_DIR definition removed, will be imported from config.py later
...
def produto_digital(ideia: Ideia, empresa_agentes: dict, todos_locais: dict):
    ...
    try:
        os.makedirs(config.PRODUTOS_GERADOS_DIR, exist_ok=True)
        logger.info(f"Diretório '{config.PRODUTOS_GERADOS_DIR}' assegurado.")
    except OSError as e:
        logger.error(f"Erro ao criar diretório '{config.PRODUTOS_GERADOS_DIR}': {e}")
        state.registrar_evento(f"Falha ao criar diretório de produtos: {e}", locals()) # Using state.registrar_evento
        return None
    ...
    try:
        ...
        if not selected_llm_model:
            ...
            state.registrar_evento("Falha ao selecionar modelo LLM para CriadorDeProdutos", locals())
            return None
        ...
        state.registrar_evento(f"Exceção ao selecionar modelo LLM: {e}", locals())
        return None
...
"""
    ...
    try:
        ...
        if not resposta_llm_content or not resposta_llm_content.strip():
            ...
            state.registrar_evento("Falha na geração de conteúdo: LLM retornou vazio.", locals())
            return None
        ...
        state.registrar_evento(f"Exceção ao chamar LLM para conteúdo: {e}", locals())
        return None
    ...
    try:
        ...
        state.registrar_evento(f"Exceção ao salvar arquivo do produto: {e}", locals())
        return None
    ...
    except ValueError as e: # This is GumroadAPIKeyError which is a subclass of ValueError
        logger.error(f"Erro ao obter chave da API Gumroad: {e}")
        state.registrar_evento(f"Falha ao obter chave Gumroad: {e}", locals())
    ...
    try:
        ...
        if product_url:
            ...
            state.registrar_evento(f"Novo produto '{ideia.descricao}' criado e publicado na Gumroad: {product_url}", todos_locais) # Pass todos_locais
            return product_url
        else:
            ...
            state.registrar_evento(f"Falha ao publicar '{ideia.descricao}' na Gumroad (retorno None).", todos_locais)
        ...
        state.registrar_evento(f"Exceção ao publicar '{ideia.descricao}' na Gumroad: {e}", todos_locais)
        # Fall through to cleanup
...
if __name__ == '__main__':
    ...
    # Mock registrar_evento (which is now state.registrar_evento)
    # This would require patching state.registrar_evento if we were to test it here via __main__
    # For pytest, we'd patch 'criador_de_produtos.state.registrar_evento' or just 'state.registrar_evento'
    # depending on how it's imported and used.
    # For this __main__ block, direct replacement is harder.
    # Let's assume for __main__ testing, we might need to adjust how registrar_evento is mocked,
    # or this __main__ block might need to be removed/rethought in light of the refactor.
    # For now, I'll comment out the mock for registrar_evento in this __main__ block.
    ...
    # global registrar_evento # registrar_evento is no longer global in this module
    # original_registrar_evento = state.registrar_evento # Assuming state is imported
    # state.registrar_evento = mock_registrar_evento
    ...
    # state.registrar_evento = original_registrar_evento # Restore original state.registrar_evento
    ...
    if os.path.exists(config.PRODUTOS_GERADOS_DIR) and not os.listdir(config.PRODUTOS_GERADOS_DIR):
        try:
            os.rmdir(config.PRODUTOS_GERADOS_DIR)
            logger.info(f"Diretório de teste '{config.PRODUTOS_GERADOS_DIR}' removido pois estava vazio.")
        except OSError as e:
            logger.warning(f"Não foi possível remover o diretório '{config.PRODUTOS_GERADOS_DIR}': {e}")
